...
test('typing, badge hold and landing finish before welcome and input',()=>{
 ...
 for(const [phase,duration] of [['knight',2200],['greetingOut',1300],['badges',3200],['departing',700],['welcome',2400],['revealing',500]]){
...
test('scroll cannot truncate typing or landing and preserves a readable badge hold',()=>{
 ...
 act(()=>jest.advanceTimersByTime(1));expect(result.current.phase).toBe('greetingOut');
 act(()=>jest.advanceTimersByTime(1300));expect(result.current.phase).toBe('badges');
 ...
 for(let i=0;i<50;i++)wheel();
 act(()=>jest.advanceTimersByTime(699));expect(result.current.phase).toBe('departing');
