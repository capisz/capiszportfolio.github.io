...
import { stackSkillByLabel } from '../../data/projectEvidence';
...
const groups = {
  ...
  'Tools & infrastructure': ['GitHub Actions','GitHub','Xcode','Visual Studio','Figma','Blender','Canva','Vite.js','Ubuntu','Linux','Kubernetes','Docker','AWS','Lambda','Webhooks','Vercel','Cloudflare','kind','Git','Claude AI','Google Gemini','Cursor AI','Codex','Postman'],
};
  ...
  return <PlaybackProvider motion={motion.enabled}><div ref={rootRef} className={`pf-root ${motion.enabled ? 'motion-on' : 'motion-off'}`}>
    ...
    <main>
      ...
      <section id="work" className="pf-section">
        ...
        {choice==='all'&&<>
        <SectionHeading label="Selected work" title={<><span className="pf-heading-accent">Actual projects</span>, not a bunch of Spotify clones</>}><a className="pf-text-link" href={links.github} target="_blank" rel="noreferrer">All repositories <Arrow /></a></SectionHeading>
      ...
      <section id="stack" className="pf-section pf-stack"><SectionHeading label="The toolkit" title={<>What I <span className="pf-heading-accent">build</span> with.</>} />{match?.requested.length>0&&<p className="pf-stack-note">What you’re looking for is highlighted</p>}<div className="pf-stack-groups">{Object.entries(groups).map(([name,names])=><div className="pf-stack-group" key={name}><h3>{name}</h3><div className="pf-stack-pills">{names.map((name,i)=>{const s=stack.find(t=>t.name===name);return <div className={`pf-stack-pill ${match?.requested.includes(stackSkillByLabel[name]||name)?'is-requested':''}`} data-reveal data-delay={(i%4)*60} key={name}><img className="pf-stack-icon" src={s.icon} alt="" /><span>{name}</span></div>;})}</div></div>)}</div></section>
